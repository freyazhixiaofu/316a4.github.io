(function () {
  ...
  const tooltip = d3
    .select("body")
    .append("div")
    .attr("class", "tooltip stack-tooltip")
    .style("opacity", 0);

  let allData = [];
  let currentGender = "All";
  let allBinStartsGlobal = [];
  let colorMode = "cartoon";
  let sortMode = "height";
  ...
  function removeRuler() {
    ...
    drawRulerMode = false;
    overlayHitbox.style("cursor", "default").style("pointer-events", "none");
    setRulerButtons();
  }
  ...
  function updateChart(gender) {
    ...
    const cells = g.selectAll(".rect-cell").data(stacked, d => `${d.age_bin}-${d.binStart}-${gender}`);
  ...
  function setupGenderButtons() {
    d3.selectAll("#stack-controls .stack-gender-toggle").on("click", function () {
      const gender = this.getAttribute("data-gender");
      d3.selectAll("#stack-controls .stack-gender-toggle").classed("active", false);
